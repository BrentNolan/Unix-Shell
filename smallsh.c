/*****************************************************************
**File Description: Shell program for Unix
...
int redirect(char **arguments, int putType, int background, int marker);
...
int main()
{
	...
	shellPID = getpid();
	shPID = shellPID;
	//printf("The shellPID is %d", shPID);
...
void loop()
{
	...
	do
	{
		...
		//printf("In main about to call execute \n");
...
int parseLine(char *buffer, char **arguments)
{
	...
	{	
		...
		arguments[counter] = strtok(NULL, " \n");
			...
			//printf("Before the & part \n");
	/*
	if(strcmp(arguments[counter], "&") == 0)
	{
		printf("In the & part");
		if(ignoreBG == 1)
		{
			printf("In the check ignoreBg part \n");
	...
	*/
	...
	//printf("Out of the and part part \n");
	
	//printf("checking last arg for $$ \n");
	char substr[] = "$$";
	//printf("counter value is %i \n", counter);
	//printf("the string in arguments[counter] is %s \n", arguments[counter]);
	
	if(counter > 0)
	{
		counter--;		
		if(strstr(arguments[counter], substr) != NULL)
	{
		//printf("In the $$ part\n");
	
	if(strcmp(arguments[counter], "$$") == 0)
		sprintf(arguments[counter], "%d",shPID);
	...
	else
	{
		...
		char newArg[50];
		char catArg[50];
		sprintf(catArg, "%d", shPID);
		pnewArg = strtok(arguments[counter], "$");
		sprintf(newArg, "%s", pnewArg);
		strcat(newArg, catArg);
		strcpy(arguments[counter], newArg);
...
int launch(char **arguments, pid_t *chList, int *lastStatus, int marker)
{
	 ...
	 */
	 //printf("In launch about to check for & \n");
...
//printf("In file io check \n");
		...
		fd = redirect(arguments, putType, background, marker);	
	}
//printf("Past file io check \n");	
...
//printf("Past signal handling \n");
		if(strcmp(arguments[0], "wc") == 0 && fd > -1)
		{
			dup2(fd, 1);
...
int redirect(char **arguments, int putType, int background, int marker)
{
	...
	if(strcmp(arguments[0], "wc") == 0)
	{
		...
		//printf("Before copy");
		//printf("Arguments 1 is %s", arguments[1]);
		//printf("Arguments 2 is %s", arguments[2]);
		sprintf(temp,"%s", arguments[2]);
		sprintf(arguments[1],"%s", temp);
		//printf("after copy");
		//printf("Arguments 1 is %s", arguments[1]);
		//printf("Arguments 2 is %s", arguments[2]);
		arguments[2] = NULL;
		
		if(strcmp(arguments[4], "junk2") ==0)
		{
			fd = open(arguments[4], O_WRONLY | O_CREAT | O_TRUNC, 0664);
			return fd;
